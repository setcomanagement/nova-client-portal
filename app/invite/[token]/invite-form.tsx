...
import { setInvitePasswordAction, type InviteState } from "./actions";

const init: InviteState = {};
const field =
  "w-full rounded-lg border border-border bg-bone px-3 py-2 text-sm outline-none focus:border-accent";

export function InviteForm({ token }: { token: string }) {
  const action = setInvitePasswordAction.bind(null, token);
  const [state, formAction, pending] = useActionState(action, init);
  return (
    <form action={formAction} className="flex flex-col gap-4">
      <label className="flex flex-col gap-1.5">
        <span className="text-xs font-medium text-muted-foreground">Choose a password</span>
      ...
      {state.error && (
        <p className="text-sm" style={{ color: "var(--clay)" }}>{state.error}</p>
      )}
      <button
        type="submit"
        disabled={pending}
